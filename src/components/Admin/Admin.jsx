...
const Admin = () => {
    const { getCategories, getProducts, products, deleteProduct, categories, addNewProduct } = useContext(ProductContext);
    ...
    const createProduct = async () => {
        const ok = await addNewProduct(data.name, data.price, data.desc, data.category, file);
        console.log(ok);
        if(ok) {
            setSearch("")
    ...
    const categoriesList = !categories ? null : categories.map((category, i) => {
        return (
    ...
    return (
        <>
            <div className='d-flex flex-column justify-content-center align-items-center mt-3 mb-3 admin-container'>
                ...
                <div className='d-flex justify-content-center'><button onClick={showModal} className='btn btn-primary'>Add new product</button></div>
                <table>
                    <thead>
                        <tr>
                            <th>Image</th>
            ...
            >
                <form className='form-product'>
                    ...
                    <div>
                        ...
                        <select className="form-select" name="category" value={data.category} onChange={handleChange}>
                            {categoriesList}
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" id="price" name="price" step=".01" className='form-control' value={data.price} onChange={handleChange}/>
                    </div>

                    <div>
                        <label htmlFor='desc'>Description:</label>
                        <textarea id="desc" rows="3" name="desc" className='form-control' value={data.desc} onChange={handleChange}></textarea>
                    </div>
