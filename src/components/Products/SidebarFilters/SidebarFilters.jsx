...
const SidebarFilters = (props) => {
    ...
    const validateMaxPrice = (e) => {
        ...
        if (input.value === "" || parseInt(input.value) > 9999) {
            e.target.value = "9999";
        } else if (parseInt(input.value) < filters.minPrice)
            input.value = filters.minPrice;
    ...
    const categoriesList = categories_filter.map((category) => (
        <li
            className={
                category.id === filters.categoryActive
                    ? "category-selected"
                    : ""
            }
            key={category.id}
            ...
            <div className="price-filter">
                ...
                <div className="inputs-price">
                    ...
                    <div className="price-order">
                        <label className="price-label mb-2 mt-2">
                            {" "}
                            Order:{" "}
                        </label>
                        <select
                            className="form-select"
                            aria-label="Order products select"
                ...
                <div className="container-rating-filter">
                    {[6, 5, 4, 3, 2, 1].map((number) => (
                        <div
                            key={number}
                            className={
                                filters.rating_filter === number
                                    ? "filter filter-selected"
                                    : "filter"
                            }
                            onClick={(e) => reviewClick(e, number)}
                        >
                            {number === 6 ? (
                                "No rating filters"
                            ) : (
                                <>
                                    {printReviewsStar(number)}{" "}
                                    <span>or more...</span>
                                </>
                            )}{" "}
                        </div>
